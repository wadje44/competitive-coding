...
const sinonChai = require('sinon-chai');// eslint-disable-line import/no-unresolved
const chaiAsPromised = require('chai-as-promised');// eslint-disable-line import/no-extraneous-dependencies
const customerinformationDao = require('../../dao/customer-information');
...
common.chai.use(sinonChai);

const sandbox = sinon.createSandbox();
common.chai.use(chaiAsPromised);

describe('Test getListCustomerInformation function', () => {
  beforeEach(() => {
  ...
  fixtures.forEach((data) => {
    it('Return all customer information', async () => {
      // ACT && ASSERT
      sandbox.stub(rest, 'apiCall').returns(
        [fullsac],
      );
      const actual = await customerinformationDao
        .getListCustomerInformation(data.input.namespace, data.input.queryParam);
      common.expect(actual).to.eql([customerInformation]);
...
describe('Test getCustomerInformationById function', () => {
  beforeEach(() => {
  ...
  it('Return all customer information by id', async () => {
    // ACT && ASSERT
    sandbox.stub(rest, 'apiCall').returns(fullsac);

    const actual = await customerinformationDao.getCustomerInformationById({ countryCode: 'ZZ', id: 'abc' });
    common.expect(actual).to.eql(customerInformation);
  });
});
