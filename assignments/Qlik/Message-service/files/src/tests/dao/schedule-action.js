...
const scheduleactionDao = require('../../dao/schedule-action');
...
describe('Test getListScheduleAction function', () => {
  beforeEach(() => {
  ...
  fixtures.forEach((data) => {
    it('Return all schedule actions', async () => {
      // ACT && ASSERT
      sandbox.stub(rest, 'apiCall').returns(
        [fullsac],
      );
      const actual = await scheduleactionDao
        .getListScheduleAction(data.input.namespace, data.input.queryParam);
      common.expect(actual).to.eql([scheduleAction]);
...
describe('Test getScheduleActionById function', () => {
  beforeEach(() => {
  ...
  it('Return schedule action by id', async () => {
    // ACT && ASSERT
    sandbox.stub(rest, 'apiCall').returns(fullsac);

    const actual = await scheduleactionDao.getScheduleActionById({ countryCode: 'ZZ', id: 'abc' }, {});
    common.expect(actual).to.eql(scheduleAction);
  });
});
